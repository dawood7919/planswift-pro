...
/**
 * One hue per takeoff type, used on the canvas, in the trees and in the reports.
 *
 * The three planar kinds share a cyan family because they all derive from an area.
 * Orange is deliberately absent: it is reserved for money and primary actions, so an
 * orange figure in this product always means currency and never a measurement.
 */
const colors: Record<MeasurementKind, string> = {
  AREA: "#22d3ee",
  ROOF_AREA: "#7dd3fc",
  VOLUME: "#0e9bb4",
  LINEAR: "#f5a524",
...
export default function WorkspacePage() {
  ...
  // Below the desktop breakpoint the side panels become drawers rather than disappearing.
  ...
  const calculateItemCost = (item: Item) => {
    ...
    const template = library.templates.find((candidate) => candidate.id === item.templateId);
    try {
      return template ? evaluateTemplateWithDependencies(template.id, library.templates, { quantity: measurement.value, rate: Number(template.rate), waste: 0 }).value : measurement.value * Number(item.rate || 0);
    } catch { return null; }
  };
  const reportRows = useMemo(() => buildQuantityReport(pageItems.map((item) => ({ ...item, template: library.templates.find((template) => template.id === item.templateId) ?? null })), calibration, library.templates), [pageItems, calibration, library.templates]);
  const projectTotal = useMemo(() => reportTotal(reportRows), [reportRows]);
  const searchResults = useMemo(() => searchTakeoffItems(pageItems.map((item) => ({ ...item, templateName: library.templates.find((template) => template.id === item.templateId)?.name ?? null })), itemSearchQuery), [pageItems, library.templates, itemSearchQuery]);
  ...
  function downloadReportCsv() {
    const csv = reportToCsv(reportRows, projectTotal, workspaceQuery.data?.project.currency ?? "");
  ...
  function downloadReportExcel() {
    const xlsx = createQuantityReportXlsx({ projectName: workspaceQuery.data?.project.name ?? "Takeoff", pageName: page?.name ?? "الصفحة", currency: workspaceQuery.data?.project.currency ?? "USD", rows: reportRows, total: projectTotal, generatedAt: new Date().toLocaleString("ar") });
  ...
  function printPageReport() {
    const html = buildPrintableReportHtml({ projectName: workspaceQuery.data?.project.name ?? "Takeoff", pageName: page?.name ?? "الصفحة", currency: workspaceQuery.data?.project.currency ?? "USD", rows: reportRows, total: projectTotal, generatedAt: new Date().toLocaleString("ar") });
  ...
  return (
    <div className="takeoff-workspace" dir="rtl" data-inspector={isInspectorOpen ? "open" : "closed"} data-navigator={isNavigatorOpen ? "open" : "closed"}>
      ...
      <div className="workspace-layout">
        <aside className="page-navigator" aria-label="الصفحات والقياسات">
          <div className="navigator-heading"><span>الصفحات، القياسات</span><span className="navigator-actions"><Button variant="ghost" size="icon" onClick={() => projectId && addBlankPage.mutate({ projectId })} aria-label="إضافة صفحة"><Plus size={15} /></Button><Button variant="ghost" size="icon" onClick={() => library.setIsOpen((current) => !current)} aria-label="إظهار القوالب"><Layers3 size={15} /></Button></span></div>
        ...
        <aside className="inspector-panel">
          ...
          <section className="ps-template-browser"><div className="ps-browser-heading"><span>القوالب، الأجزاء، المدخلات</span><Button variant="ghost" size="icon" onClick={() => library.setIsOpen((current) => !current)} aria-label="فتح إدارة القوالب"><Plus size={14} /></Button></div><div className="ps-browser-tabs"><span className="active">Parts & Assemblies</span><span>Inputs</span></div><div className="ps-template-tree">{library.templates.length ? library.templates.map((template) => <button key={template.id} type="button" className={library.editingTemplateId === template.id ? "active" : ""} onClick={() => { library.edit(template); library.setIsOpen(true); }}><i style={{ background: template.kind === "ASSEMBLY" ? "#e88355" : "#4f9fd0" }} /><span><b>{template.name}</b><small>{template.kind} · {template.unit}</small></span><em>{template.costItems?.length ?? 0}</em></button>) : <p>أضف Parts وAssemblies لتظهر هنا.</p>}</div></section>
          ...
          {library.isOpen && <section className="estimate-library-card"><span className="panel-kicker">ESTIMATING LIBRARY</span><h2>مواد وعمالة القالب</h2><p>نظّم القوالب في مجلدات وأضف بنود تكلفة مستقلة يمكن مراجعتها في التقرير.</p><div className="template-folder-controls"><Input value={library.folderName} onChange={(event) => library.setFolderName(event.target.value)} placeholder="اسم مجلد جديد" maxLength={120} /><Button type="button" variant="outline" size="sm" disabled={!library.folderName.trim() || library.createFolder.isPending} onClick={() => library.createFolder.mutate({ name: library.folderName.trim() })}>إضافة مجلد</Button></div><Label>مجلد القالب<select value={library.folderId} onChange={(event) => library.setFolderId(event.target.value)}><option value="">بدون مجلد</option>{library.folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></Label>{library.folderId && !library.editingTemplateId && <small>سيُربط المجلد عند حفظ القالب الجديد.</small>}{library.folderId && library.editingTemplateId && <Button type="button" variant="outline" size="sm" onClick={() => library.deleteFolder.mutate(library.folderId)} disabled={library.deleteFolder.isPending}>حذف المجلد الفارغ</Button>}{library.editingTemplateId ? <div className="cost-items-editor"><h3>بنود «{library.templates.find((template) => template.id === library.editingTemplateId)?.name ?? "القالب"}»</h3>{library.templates.find((template) => template.id === library.editingTemplateId)?.costItems.length ? <div className="cost-items-list">{library.templates.find((template) => template.id === library.editingTemplateId)?.costItems.map((item) => <div key={item.id}><span><b>{item.name}</b><small>{item.kind} · {item.quantityFormula} × {item.rate}{Number(item.wastePercent) ? ` · هالك ${item.wastePercent}%` : ""}</small></span><button type="button" aria-label={`حذف بند ${item.name}`} onClick={() => library.deleteCostItem.mutate(item.id)}><Trash2 size={13} /></button></div>)}</div> : <small>لا توجد بنود تكلفة بعد؛ تبقى تكلفة القالب الأساسية كما هي.</small>}<form className="cost-item-form" onSubmit={(event) => { event.preventDefault(); library.submitCostItem(); }}><select value={library.costItemKind} onChange={(event) => library.setCostItemKind(event.target.value as "MATERIAL" | "LABOR" | "EQUIPMENT")}><option value="MATERIAL">مادة</option><option value="LABOR">عمالة</option><option value="EQUIPMENT">معدات</option></select><Input value={library.costItemName} onChange={(event) => library.setCostItemName(event.target.value)} placeholder="اسم البند" maxLength={160} /><Input value={library.costItemQuantityFormula} onChange={(event) => library.setCostItemQuantityFormula(event.target.value)} placeholder="صيغة الكمية: quantity" maxLength={500} /><div className="template-form-grid"><Input value={library.costItemUnit} onChange={(event) => library.setCostItemUnit(event.target.value)} placeholder="وحدة البند" maxLength={32} /><Input value={library.costItemRate} onChange={(event) => library.setCostItemRate(event.target.value)} inputMode="decimal" placeholder="سعر الوحدة" /><Input value={library.costItemWastePercent} onChange={(event) => library.setCostItemWastePercent(event.target.value)} inputMode="decimal" placeholder="هالك %" /></div><Button type="submit" className="workspace-primary full" disabled={library.createCostItem.isPending}>{library.createCostItem.isPending ? "جارٍ الإضافة" : "إضافة بند تكلفة"}</Button></form></div> : <small>اختر قالباً من القائمة ثم اضغط تحرير لإضافة مواده أو عمالته.</small>}</section>}
          ...
          {library.isOpen && <section className="templates-card"><span className="panel-kicker">PARTS & ASSEMBLIES</span><h2>مكتبة القوالب</h2><div className="template-list">{library.templates.length ? library.templates.map((template) => <div key={template.id} className="template-row"><button type="button" className="template-edit" onClick={() => library.edit(template)}><b>{template.name}</b><small>{template.kind} · {template.formula}{template.dependencyIds.length ? ` · ${template.dependencyIds.length} مراجع` : ""}</small></button><button type="button" onClick={() => library.deleteTemplate.mutate(template.id)} disabled={library.deleteTemplate.isPending} aria-label={`حذف ${template.name}`}><Trash2 size={13} /></button></div>) : <p>أنشئ أول Part أو Assembly للاستخدام في عناصر القياس.</p>}</div><form className="template-form" onSubmit={(event) => { event.preventDefault(); library.submit(); }}><div className="template-form-heading"><b>{library.editingTemplateId ? "تعديل القالب" : "قالب جديد"}</b>{library.editingTemplateId && <Button type="button" variant="outline" size="sm" onClick={library.resetForm}>قالب جديد</Button>}</div><select value={library.kind} onChange={(event) => library.setKind(event.target.value as "PART" | "ASSEMBLY")} disabled={!!library.editingTemplateId}><option value="PART">Part</option><option value="ASSEMBLY">Assembly</option></select><Input value={library.name} onChange={(event) => library.setName(event.target.value)} placeholder="اسم القالب" maxLength={160} /><Input value={library.formula} onChange={(event) => library.setFormula(event.target.value)} placeholder="quantity * rate" maxLength={500} /><small className={`formula-diagnostic ${library.formulaDiagnostic.valid ? "valid" : "invalid"}`} role="status">{library.formulaDiagnostic.message}</small><div className="template-form-grid"><Input value={library.unit} onChange={(event) => library.setUnit(event.target.value)} placeholder="الوحدة الناتجة (مثل m²)" maxLength={32} /><Input value={library.rate} onChange={(event) => library.setRate(event.target.value)} inputMode="decimal" placeholder="السعر" /></div><fieldset className="template-dependencies"><legend>مراجع القالب</legend><p>تُضاف قيمة كل قالب محدد إلى ناتج القالب الحالي بترتيب حتمي.</p>{library.templates.filter((template) => template.id !== library.editingTemplateId).length ? library.templates.filter((template) => template.id !== library.editingTemplateId).map((template) => <label key={template.id}><input type="checkbox" checked={library.dependencyIds.includes(template.id)} onChange={(event) => library.setDependencyIds((current) => event.target.checked ? [...current, template.id] : current.filter((id) => id !== template.id))} />{template.name}</label>) : <small>احفظ قالباً آخر أولاً ليصبح مرجعاً متاحاً.</small>}</fieldset><small className={`formula-diagnostic ${library.dependencyDiagnostic.valid ? "valid" : "invalid"}`} role="status">{library.dependencyDiagnostic.message}{library.dependencyDiagnostic.cycleIds.length ? ` (${library.dependencyDiagnostic.cycleIds.join(" ← ")})` : ""}</small><Button className="workspace-primary full" type="submit" disabled={library.createTemplate.isPending || library.updateTemplate.isPending || !library.formulaDiagnostic.valid || !library.dependencyDiagnostic.valid}>{library.editingTemplateId ? "حفظ التعديل" : "حفظ القالب"}</Button></form><small className="template-hint">المتغيرات المتاحة: quantity وrate وwaste. العمليات المسموح بها: + − × ÷ والأقواس. المراجع تضيف نواتج القوالب المختارة ولا تسمح بالدورات.</small></section>}
