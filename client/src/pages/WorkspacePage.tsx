    ...
    <div className="takeoff-workspace" dir={direction}>
      ...
      <div className="workspace-layout">
        ...
        <aside className="inspector-panel">
          ...
          <section className="annotation-card"><span className="panel-kicker">MARKUP NOTE</span><h2>{t("workspace.annotation.title")}</h2><p>{t("workspace.annotation.description")}</p><Input value={annotationText} onChange={(event) => setAnnotationText(event.target.value)} placeholder={t("workspace.annotation.placeholder")} maxLength={500} /><div className="annotation-controls"><Input aria-label={t("workspace.annotation.color")} type="color" value={annotationColor} onChange={(event) => setAnnotationColor(event.target.value)} /><Button type="button" variant={tool === "ANNOTATE" ? "default" : "outline"} size="sm" onClick={() => setTool("ANNOTATE")} disabled={!annotationText.trim()}>{t("workspace.annotation.place")}</Button></div>{pageAnnotations.length ? <div className="annotation-list">{pageAnnotations.map((annotation) => <div key={annotation.id}><span style={{ color: annotation.color }}>{annotation.text}</span><button type="button" aria-label={`${t("workspace.annotation.delete")} ${annotation.text}`} onClick={() => projectId && deleteAnnotation.mutate({ projectId, annotationId: annotation.id })}><Trash2 size={13} /></button></div>)}</div> : <small>{t("workspace.annotation.empty")}</small>}</section>
          {selected && selectedReportRow ? <section className="multiplier-card"><span className="panel-kicker">TYPICAL TAKEOFF</span><h2>{t("workspace.multiplier.title")}</h2><Label>{t("workspace.multiplier.units")}<Input value={selected.multiplier} inputMode="decimal" onChange={(event) => updateSelectedMultiplier(event.target.value)} onBlur={commitSelectedMultiplier} aria-label={t("workspace.multiplier.itemAria")} /></Label><small>{t("workspace.multiplier.geometryQuantity")} {selectedReportRow.quantity?.toFixed(2) ?? "—"} {selectedReportRow.unit}</small></section> : null}
          {selectedReportRow?.costBreakdown.length ? <section className="cost-audit-card"><span className="panel-kicker">COST AUDIT</span><h2>{t("workspace.costAudit.title")}</h2><div>{selectedReportRow.costBreakdown.map((entry) => <article key={entry.id}><span><b>{entry.name}</b><small>{entry.kind} · {entry.quantity.toFixed(2)} {entry.unit} × {entry.rate}{entry.wastePercent ? ` · ${t("workspace.costAudit.waste")} ${entry.wastePercent}%` : ""}</small></span><strong>{new Intl.NumberFormat("en-US", { style: "currency", currency: workspaceQuery.data?.project.currency ?? "USD" }).format(entry.cost)}</strong></article>)}</div></section> : null}
