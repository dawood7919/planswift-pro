...
describe("PdfPlanLayer failure recovery", () => {
  ...
  it("renders the actual error overlay and retries after a PDF load failure", async () => {
    ...
    render(createElement(PdfPlanLayer, { url: "/invalid.pdf", pageNumber: 1, zoom: 1, pan: { x: 0, y: 0 } }));
  ...
  it("renders a successful PDF page behind the shared viewport transform", async () => {
    ...
    const cleanup = vi.fn();
    pdfMocks.getDocument.mockImplementation(() => ({ promise: Promise.resolve({ getPage: vi.fn(() => Promise.resolve({ getViewport: vi.fn(() => ({ width: 320, height: 480 })), render: pageRender })), cleanup }), destroy: vi.fn() }));
    vi.spyOn(HTMLCanvasElement.prototype, "getContext").mockReturnValue({} as CanvasRenderingContext2D);
    const view = render(createElement(PdfPlanLayer, { url: "/valid.pdf", pageNumber: 2, zoom: 1.5, pan: { x: 10, y: -5 } }));
    ...
    expect(view.container.querySelector(".pdf-plan-transform")?.getAttribute("style")).toContain("matrix(");
